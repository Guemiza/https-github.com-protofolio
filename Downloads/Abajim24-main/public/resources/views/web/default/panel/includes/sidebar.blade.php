...
<div class="panel-sidebar pt-50 pb-25 px-25" style="z-index:9!important" id="panelSidebar">
    ...
    <div class="user-info d-flex align-items-center flex-row flex-lg-column justify-content-lg-center">
        ...
        --}}
            ...
            @if(!empty($matiereNames))

            <p class="font-13">
                @if (count($matiereNames) == 1)
                مادة : {{ $matiereNames[0] }}
                @elseif (count($matiereNames) == 2)
                مادتين: {{ $matiereNames[0] }} و {{ $matiereNames[1] }}
                @else
                المواد :
                @foreach ($matiereNames as $key => $name)
                {{ $name }}
                @if ($key < count($matiereNames) - 1) و @endif @endforeach @endif </p>
    ...
    <ul id="panel-sidebar-scroll" class="sidebar-menu pt-10 @if(!empty($authUser->userGroup)) has-user-group @endif @if(empty($getPanelSidebarSettings) or empty($getPanelSidebarSettings['background'])) without-bottom-image @endif" @if((!empty($isRtl) and $isRtl)) data-simplebar-direction="rtl" @endif>
        ...
        @endif
        {{-- @if($authUser->isUser())
         <li class="sidenav-item {{ (request()->is('panel/init/scolaire/1')) ? 'sidenav-item-active' : '' }}">
        <a href="/panel/init/scolaire/78" class="d-flex align-items-center" class="d-flex align-items-center">
            <span class="sidenav-item-icon mr-10">
        ...
        <li class="sidenav-item {{ (request()->is('panel/scolaire/teacher/1')) ? 'sidenav-item-active' : '' }}">
        ...
        <div class="collapse {{ (request()->is('panel/webinars') or request()->is('panel/webinars/*')) ? 'show' : '' }}" id="webinarCollapse">
            <ul class="sidenav-item-collapse">
                ...
                @if($authUser->isUser() )
                <li class="mt-5 {{ (request()->is('panel/webinars/allcourses')) ? 'active' : '' }}">
                    <a href="/panel/webinars/allcourses">All Courses</a>
                </li>
        ...
        <li class="sidenav-item {{ (request()->is('panel/meetings') or request()->is('panel/meetings/*')) ? 'sidenav-item-active' : '' }}">
            ...
            <div class="collapse {{ (request()->is('panel/meetings') or request()->is('panel/meetings/*')) ? 'show' : '' }}" id="meetingCollapse">
                <ul class="sidenav-item-collapse">
                    @if($authUser->isOrganization() ||$authUser->isUser() )
    ...
    height:30px;" src="https://upload.wikimedia.org/wikipedia/commons/thumb/6/6f/Logo_of_Twitter.svg/512px-Logo_of_Twitter.svg.png?20220821125553/">
            ...
            {{-- @foreach($socials as $social)
                  <a  style="    width: 100px;
    z-index: 9999999999!important;
    height:100px;" href="{{ $social['link'] }}">
            <img style="    width: 70px;
    z-index: 9999999999!important;
    height:40px;" src="{{ $social['image'] }}" alt="{{ $social['title'] }}" class="mr-15">
            </a>
            @endforeach --}}
