...
function INIT_AUTO_HK(st){
        ...
        box[9]=FExtension.store.get('SL_SO_wpt');
        box[10]=FExtension.store.get('SL_CT_wpt');
	...
	var T = 0;
	if(FExtension.store.get("SL_SObox_wpt") == "true") T = "1";
	box[9]=T+"|"+box[9];

	var T = 0;
	if(FExtension.store.get("SL_CTbox_wpt") == "true") T = "1";
	box[10]=T+"|"+box[10];
	...
	TT=0;
	if(st==12){
          if(GEBI('IMT_HK12').checked==true) TT=1;
	  VERbox=TT+"|"+GEBI('SRV12').value;
        }
	TT=0;
	if(st==13){
          if(GEBI('IMT_HK13').checked==true) TT=1;
	  VERbox=TT+"|"+GEBI('SRV13').value;
        }
...
//        if(st>2) cnt=-2;
        ...
        if(st==3 || st == 4){
        	...
        	box[6]=GEBI('SRV11').value;
        } else {
	        box[4]=FExtension.store.get('SL_HK_it1');
        	box[5]=FExtension.store.get('SL_HK_it2');
        	box[6]=FExtension.store.get('SL_change_lang_HK_it');
        }
        if(st==5){
        	box[7]=GEBI('SRV5').value;
        } else {
	        box[7]=FExtension.store.get('SL_HK_bb1');
        }


        if(st==6 || st == 7 || st == 12 || st == 13){
	        box[8]=GEBI('SRV6').value;
        	box[9]=GEBI('SRV7').value;
        	box[10]=GEBI('SRV12').value;
        	box[11]=GEBI('SRV13').value;
        } else {
	        box[8]=FExtension.store.get('SL_HK_wpt1');
        	box[9]=FExtension.store.get('SL_HK_wpt2');
        	box[10]=FExtension.store.get('SL_HK_SO_wpt');
        	box[11]=FExtension.store.get('SL_HK_CT_wpt');
        }

        box[12]=FExtension.store.get('SL_HK_opt');
       	box[13]=FExtension.store.get('SL_HK_btn');
...
//             alert(box[j]+" == "+box[i]+" && "+j +" != "+ i);
	...
	//for WPT
	if(box[8]==box[9] || box[8]==box[12] || box[8]==box[13] || box[9]==box[12] || box[9]==box[13] || box[12]==box[13]) cnt++;
