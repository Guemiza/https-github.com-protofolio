			...
			ajaxRequest.onreadystatechange = function(){
				if(ajaxRequest.readyState == 4){
					...
					if(resp.indexOf('trans":')!=-1){
						 ...
						 if(fromHistory==0) SL_Flip(SL_DETECT);
     ...
     if(GEBI('SLlocpl').checked==false || GEBI('SL_langSrc').value=="auto"){
      if(localStorage["SL_no_detect"]=="true" || GEBI('SL_langSrc').value=="auto"){
		...
		GEBI("SL_langSrc").value=DETECTEDlng;
	        FExtension.store.set("SL_langDst2", OLD_FROM);
	    }
	 }
       }   
      }
		...
		for(j = 0; j < T2.length; j++){
			...
			if(R[1] != undefined){
				...
				for(k = 0; k < R2.length; k++){
					...
					if(k < (R2.length-1))	Rline = Rline + "<a class=_ALNK href='dictionary.html?dir="+ F + "|" + T +"&text=" + encodeURIComponent(tmpLNK) + "'>" + R2[k] + "</a>, ";
					else			Rline = Rline + "<a class=_ALNK href='dictionary.html?dir="+ F + "|" + T +"&text=" + encodeURIComponent(tmpLNK) + "'>" + R2[k] + "</a>";
				}
...
function GoToTranslator(){
	   ...
	   if(FExtension.store.get("SL_BACK_VIEW")==2) window.location.href="../../html/popup/translation-back.html?text="+encodeURIComponent(TEXT)+"&t=1";
