	 ...
	 if(frame) {
		frame.parentNode.removeChild(frame);
		 ...
		 if(BACK_VIEW==1) GEBI('SL_back').style.height = (GLOB_BACK_HEIGHT)+"px"; 
			...
			if(GLOB_BOX==1){
				GEBI("SL_fieldset").style.height=(GLOB_BACK_HEIGHT-55)+"px";
				GEBI('SL_source').style.height = (GLOB_BACK_HEIGHT-55)+"px";
			}else{
				GEBI("SL_fieldset_trg").style.height=(GLOB_BACK_HEIGHT-17)+"px";
				GEBI('SL_target').style.height = (GLOB_BACK_HEIGHT-22)+"px";
			}
		 }
...
function SLShowHideAlert100(act,box){
	GEBI('SL_posterSRC').style.display='none';
	GEBI('SL_posterDST').style.display='none';
	var id = "SL_poster"+box;
...
function RESIZE_AND_SHOW_PLAYER(box){
	if(box==1){
		GEBI("SL_fieldset").style.height=(GLOB_BACK_HEIGHT-93)+"px";
		GEBI('SL_source').style.height = (GLOB_BACK_HEIGHT-93)+"px";
	} else {
		GEBI("SL_fieldset_trg").style.height=(GLOB_BACK_HEIGHT-45)+"px";
