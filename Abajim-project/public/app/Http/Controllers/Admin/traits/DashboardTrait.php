...
trait DashboardTrait
{
    ...
    public function getIncomeStatisticsSchool()
    {

        $dateStartAndEnd = $this->getAllDateStartAndEndSchool();
        ...
        $totalSales = $this->getIncomesSchool();

        $todaySales = $this->getIncomesSchool($beginOfDay, $endOfDay);

        $monthSales = $this->getIncomesSchool($beginOfMonth, $endOfMonth);

        $yearSales = $this->getIncomesSchool($beginOfYear, $endOfYear);
    ...
    private function getIncomesSchool($from = null, $to = null)
    {
        // $query = Accounting::where(function ($query) {
        //     $query->where('system', true)->where('type_account', 'subscribe');
        // });
    ...
    private function getAllDateStartAndEndSchool()
    {
        $time = time();
        $beginOfDay = strtotime("today", $time);
        $endOfDay = strtotime("tomorrow", $beginOfDay) - 1;
    ...
    public function getTotalSalesStatisticsSchool()
    {
        $dateStartAndEnd = $this->getAllDateStartAndEndSchool();
        ...
        $totalSales = Sale::whereNull('refund_at')->where('type', 'subscribe')->where('seller_id', auth()->user()->id)->count();
    ...
    public function getNewSalesCountSchool()
    {
        return Sale::whereNull('refund_at')
        ->where('seller_id', auth()->user()->id)
        ->where('type', 'subscribe')
    ...
    public function getNotSubscribeCountSchool()
    {
        $all_kids_in_school=User::where('school_id',auth()->user()->id)->count();
        $subscrive_kids_in_school=Sale::where('seller_id',auth()->user()->id)->whereNotNull('buyer_id')
        ->count(); //->distinct('buyer_id')
        return $all_kids_in_school-$subscrive_kids_in_school ;
    }
    public function getSubscribeCountSchool()
    {
        $subscrive_kids_in_school=Sale::where('seller_id',auth()->user()->id)->whereNotNull('buyer_id')
        ->count();

        return $subscrive_kids_in_school;
    }
    ...
    public function getMonthAndYearSalesChartSchool($type = 'month_of_year')
    {
        $labels = [];
        $data = [];
    ...
    public function getMonthAndYearSalesChartStatisticsSchool()
    {
        $dateStartAndEnd = $this->getAllDateStartAndEndSchool();
